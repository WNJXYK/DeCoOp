...
def metrics_old(predicts, targets, classnames, base_classnames, all_classnames):
    ...
    sample_selector = np.array(sample_selector)
    class_selector  = np.array(class_selector)
    base_total, new_total = np.sum(sample_selector), np.sum(~sample_selector)
    base_predicts, new_predicts = predicts.copy(), predicts.copy()
    base_predicts[:, ~class_selector] = -1e4
...
def metrics_new(predicts, targets, classnames, base_classnames, all_classnames):
    base_correct, base_total = 0, 0
    sample_selector = [ classnames[i] in base_classnames     for i in range(len(classnames)) ]
    class_selector  = [ all_classnames[i] in base_classnames for i in range(len(all_classnames)) ]
    sample_selector = np.array(sample_selector)
...
def clip_classifier(classnames, template, clip_model, norm=True):
    with torch.no_grad():
        clip_weights = []
        ...
        for classname in classnames:
            ...
            texts = clip.tokenize(texts, context_length=77).cuda()
